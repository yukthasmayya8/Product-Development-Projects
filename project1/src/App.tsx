...
import MotivationDisplay from "@/components/wellbeing/MotivationDisplay";
import TimetableDisplay from "@/components/study/TimetableDisplay";
import HistoryChart from "@/components/study/HistoryChart";
import StudyTools from "@/components/study/StudyTools";
import GeminiAssistant from "@/components/assistant/GeminiAssistant";
import DigitalBalance from "@/components/wellbeing/DigitalBalance";
import FocusAnalytics from "@/components/study/FocusAnalytics";
import DistractionLogger from "@/components/wellbeing/DistractionLogger";
import ProfileSettings from "@/components/layout/ProfileSettings";
import MindGym from "@/components/wellbeing/MindGym";
import StudyHistory from "@/components/study/StudyHistory";
import { AchievementsAndGoals } from "@/components/study/AchievementsAndGoals";
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "analytics" | "history" | "mindgym" | "tools" | "achievements"
  >("dashboard");

  const t =
    translations[userData?.preferredLanguage || "English"] ||
  ...
  const updateStreak = async () => {
    ...
    const lastStudy = userData.lastStudyDate
      ? new Date(userData.lastStudyDate.seconds * 1000)
      : null;
    ...
    try {
      ...
      if (newStreak % 5 === 0) {
        sendNotification(
          "Streak Milestone!",
          `You've reached a ${newStreak} day study streak! Keep it up!`,
        );
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-[#0a0502] flex flex-col items-center justify-center p-4 font-serif overflow-hidden">
        ...
        >
          ...
          <h1 className="text-6xl font-display uppercase tracking-tighter mb-6 text-white relative z-10">
            {translations.English.betterYou}
          </h1>
          <ABTest
            ...
            variants={{
              A: (
                <p className="text-xl font-light opacity-60 mb-16 leading-relaxed italic relative z-10 px-4">
                  ...
                  <span className="block mt-4 text-[10px] uppercase tracking-[0.4em] font-bold not-italic opacity-40">
                    — Mark Twain
                  </span>
                </p>
              ),
          ...
          >
            ...
            <span className="text-lg uppercase tracking-widest">
              {translations.English.enterSanctuary}
            </span>
          </button>
          ...
          <div className="flex items-center bg-white/5 p-2 rounded-[40px] border border-white/10 order-1 md:order-2 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
            ...
            {[
              ...
              >
                <tab.icon
                  size={16}
                  className={activeTab === tab.id ? "text-white" : "opacity-40"}
                />
...
function SetupScreen({
  ...
  const [weakSubjects, setWeakSubjects] = useState<string[]>(
    initialData?.weakSubjects || [],
  );
  const [strongSubjects, setStrongSubjects] = useState<string[]>(
    initialData?.strongSubjects || [],
  );
  ...
  return (
    <div
      className={`min-h-screen bg-[#0a0502] flex items-center justify-center p-4 font-serif ${onCancel ? "bg-transparent min-h-0 p-0" : ""}`}
    >
      ...
      >
        ...
        <h2 className="text-4xl font-display uppercase tracking-tighter mb-4">
          {t.initialisation}
        </h2>
        <p className="text-lg opacity-40 mb-12 italic">
