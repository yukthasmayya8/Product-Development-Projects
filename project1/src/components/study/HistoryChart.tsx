      ...
      <div className="relative z-10 flex flex-col h-full">
            ...
            >
              ...
              <Bar dataKey="duration" radius={[12, 12, 0, 0]} barSize={32}>
                {chartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={index % 2 === 0 ? "#ff4e00" : "rgba(255,78,0,0.4)"}
                  />
                ))}
              </Bar>
            </BarChart>
        ...
        <div className="mt-10 pt-10 border-t border-white/5 flex justify-between items-center">
          <div className="flex flex-col">
            <span className="text-[9px] font-bold uppercase tracking-[0.3em] opacity-30 mb-2">
              {t.totalFocus}
            </span>
            <span className="text-4xl font-display tracking-tighter text-white">
              {data.reduce((acc, curr) => acc + curr.durationMinutes, 0)}{" "}
              <span className="text-xs opacity-30 font-serif lowercase italic">
                {t.minutes}
              </span>
            </span>
          </div>
          <div className="flex flex-col items-end">
            ...
            <span className="text-4xl font-display tracking-tighter text-[#ff4e00]">
              {data.length}
            </span>
          </div>
        </div>
