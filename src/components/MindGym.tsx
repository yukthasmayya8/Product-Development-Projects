...
import { Gamepad2, Trophy, Lock, RefreshCw, Brain, Puzzle, Sparkles, X, CheckCircle2, AlertCircle, Timer, Star, Award, ChevronRight } from 'lucide-react';
...
const SudokuGame = ({ onComplete, onCancel, stats, updateStats }: { 
  onComplete: (time: number, difficulty: Difficulty) => void, 
  onCancel: () => void,
  stats: any,
  updateStats: (game: 'sudoku' | 'focusFlow', time: number, difficulty: Difficulty) => void
}) => {
  ...
  const handleNumberInput = (num: number) => {
    ...
    const isFull = newGrid.every((row, ri) => row.every((cell, ci) => cell === solution[ri][ci]));
    if (isFull) {
      setIsComplete(true);
      setIsActive(false);
      sounds.complete();
    ...
    return (
      <div className="text-center space-y-8 py-10">
        ...
        <div className="space-y-4">
          <h3 className="text-2xl font-display uppercase tracking-widest">BetterYou Sudoku Guide</h3>
          <p className="text-xs opacity-60 leading-relaxed max-w-xs mx-auto">
            Fill the 9x9 grid so that each row, column, and 3x3 section contains all digits from 1 to 9.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-4 max-w-xs mx-auto">
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
            ...
            <p className="text-[10px] uppercase tracking-wider font-bold opacity-70">Select an empty cell</p>
          </div>
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
            <div className="w-8 h-8 rounded-lg bg-blue-500/20 flex items-center justify-center text-blue-400 font-bold">2</div>
            <p className="text-[10px] uppercase tracking-wider font-bold opacity-70">Choose a number from the bottom</p>
          </div>
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
            <div className="w-8 h-8 rounded-lg bg-blue-500/20 flex items-center justify-center text-blue-400 font-bold">3</div>
            <p className="text-[10px] uppercase tracking-wider font-bold opacity-70">Complete the grid as fast as possible</p>
          </div>
        </div>
        <button 
    ...
    return (
      ...
      >
        ...
        <div className="space-y-2">
          <h3 className="text-3xl font-display uppercase tracking-tighter">Training Complete</h3>
          <p className="text-[10px] uppercase tracking-[0.4em] font-bold opacity-40">Cognitive Reset Successful</p>
        </div>

        <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Time Taken</p>
            <p className="text-2xl font-display font-black text-blue-400">{formatTime(timer)}</p>
          </div>
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Difficulty</p>
            <p className="text-2xl font-display font-black text-purple-400">{difficulty}</p>
          </div>
        </div>
    ...
    <div className="space-y-8 max-w-md mx-auto">
      <div className="flex justify-between items-center bg-white/5 p-4 rounded-3xl border border-white/10">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-mono font-black text-blue-400" aria-label={`Time elapsed: ${formatTime(timer)}`}>{formatTime(timer)}</span>
        </div>
      </div>

      <div className="grid grid-cols-9 gap-1 bg-white/5 p-2 rounded-[24px] border border-white/10 shadow-2xl relative" role="grid" aria-label="Sudoku board">
      ...
      <div className="grid grid-cols-9 gap-2" role="group" aria-label="Number input">
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <button onClick={onCancel} className="text-[9px] uppercase tracking-[0.4em] font-black opacity-30 hover:opacity-100 transition-all">Quit Training</button>
      </div>
    </div>
  );
};
...
const FocusFlowGame = ({ onComplete, onCancel, stats, updateStats }: { 
  onComplete: (time: number, difficulty: Difficulty) => void, 
  onCancel: () => void,
  stats: any,
  updateStats: (game: 'sudoku' | 'focusFlow', time: number, difficulty: Difficulty) => void
  ...
  const cols = difficulty === 'Easy' ? 'grid-cols-3' : difficulty === 'Medium' ? 'grid-cols-4' : 'grid-cols-5';
  ...
  const handleNumberClick = (num: number) => {
    if (num === nextNumber) {
      ...
      if (num === gridSize) {
        ...
        const bestTimeKey = `bestTime${difficulty}` as keyof typeof stats.focusFlow;
  ...
  if (showInstructions) {
    return (
      <div className="text-center space-y-8 py-10">
        ...
        <div className="space-y-4">
          <h3 className="text-2xl font-display uppercase tracking-widest">Focus Flow Guide</h3>
          <p className="text-xs opacity-60 leading-relaxed max-w-xs mx-auto">
            Find and click the numbers in ascending order as quickly as possible.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-4 max-w-xs mx-auto">
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
            <div className="w-8 h-8 rounded-lg bg-purple-500/20 flex items-center justify-center text-purple-400 font-bold">1</div>
            <p className="text-[10px] uppercase tracking-wider font-bold opacity-70">Locate the target number</p>
          </div>
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
            <div className="w-8 h-8 rounded-lg bg-purple-500/20 flex items-center justify-center text-purple-400 font-bold">2</div>
            <p className="text-[10px] uppercase tracking-wider font-bold opacity-70">Click to advance to the next</p>
          </div>
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 text-left">
            <div className="w-8 h-8 rounded-lg bg-purple-500/20 flex items-center justify-center text-purple-400 font-bold">3</div>
            <p className="text-[10px] uppercase tracking-wider font-bold opacity-70">Clear the entire grid</p>
          </div>
        </div>
        <button 
          onClick={() => { sounds.click(); setShowInstructions(false); }}
  ...
  if (showSummary) {
    return (
      ...
      >
        ...
        <div className="space-y-2">
          <h3 className="text-3xl font-display uppercase tracking-tighter">Focus Restored</h3>
          <p className="text-[10px] uppercase tracking-[0.4em] font-bold opacity-40">Cognitive Reset Successful</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Time Taken</p>
            <p className="text-2xl font-display font-black text-purple-400">{formatTime(timer)}</p>
          </div>
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Difficulty</p>
            <p className="text-2xl font-display font-black text-blue-400">{difficulty}</p>
          </div>
        </div>
    ...
    <div className="space-y-8 max-w-md mx-auto">
      <div className="flex justify-between items-center bg-white/5 p-4 rounded-3xl border border-white/10">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-mono font-black text-purple-400" aria-label={`Time elapsed: ${formatTime(timer)}`}>{formatTime(timer)}</span>
        </div>
      </div>

      <div className="flex justify-between items-center px-4">
        <div className="flex flex-col">
          <span className="text-[10px] uppercase tracking-[0.4em] font-black opacity-30">Target</span>
          <span className="text-3xl font-display text-purple-400 font-black">{nextNumber}</span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-[10px] uppercase tracking-[0.4em] font-black opacity-30">Max</span>
          <span className="text-3xl font-display text-white/40 font-black">{gridSize}</span>
        </div>
      </div>

      <div className={`grid ${cols} gap-4 p-4 bg-white/5 rounded-[32px] border border-white/10 shadow-2xl relative`} role="grid" aria-label="Focus Flow grid">
        {numbers.map((num) => (
          <motion.button
            key={num}
            ...
            className={`aspect-square rounded-2xl flex items-center justify-center text-xl font-display font-black transition-all border
              ${num < nextNumber ? 'bg-green-500/20 border-green-500/40 text-green-400 opacity-40' : 
                num === nextNumber ? 'bg-gradient-to-br from-purple-600 to-purple-400 border-purple-300 text-white shadow-2xl glow-purple scale-110 z-20' : 
...
export default function MindGym({ userId, language = 'English' }: MindGymProps) {
  const [hasPlayedToday, setHasPlayedToday] = useState(false);
  const [loading, setLoading] = useState(true);
  const [currentGame, setCurrentGame] = useState<'sudoku' | 'puzzle' | null>(null);
  ...
  const fetchStats = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        const initialStats = {
          userId,
          sudoku: { gamesPlayed: 0, bestTimeEasy: null, bestTimeMedium: null, bestTimeHard: null },
          focusFlow: { gamesPlayed: 0, bestTimeEasy: null, bestTimeMedium: null, bestTimeHard: null }
        };
  ...
  const updateStats = async (game: 'sudoku' | 'focusFlow', time: number, difficulty: Difficulty) => {
    if (!stats) return;
    ...
    if (gameStats[bestTimeKey] === null || time < (gameStats[bestTimeKey] as number)) {
  ...
  const checkPlayStatus = async () => {
    try {
      ...
      handleFirestoreError(error, OperationType.GET, `gameLogs/${userId}_${new Date().toISOString().split('T')[0]}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-10">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-display uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#ff4e00] via-purple-500 to-blue-500">{t.mindGym}</h2>
          <p className="text-[10px] uppercase tracking-widest font-bold opacity-40">{t.cognitiveReset}</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-2xl border border-white/10">
            <Trophy size={16} className="text-yellow-500" />
            <span className="text-xs font-bold uppercase tracking-widest">{t.dailyLimit}</span>
          </div>
        </div>
      </div>
      ...
      {stats && (
        ...
        >
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Total Games</p>
            <p className="text-2xl font-display font-black">{(stats.sudoku?.gamesPlayed || 0) + (stats.focusFlow?.gamesPlayed || 0)}</p>
          </div>
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Sudoku Best</p>
            <p className="text-2xl font-display font-black text-blue-400">
              {stats.sudoku?.bestTimeEasy ? `${Math.floor(stats.sudoku.bestTimeEasy / 60)}:${(stats.sudoku.bestTimeEasy % 60).toString().padStart(2, '0')}` : '--:--'}
            </p>
          </div>
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Focus Best</p>
            <p className="text-2xl font-display font-black text-purple-400">
              {stats.focusFlow?.bestTimeEasy ? `${Math.floor(stats.focusFlow.bestTimeEasy / 60)}:${(stats.focusFlow.bestTimeEasy % 60).toString().padStart(2, '0')}` : '--:--'}
            </p>
          </div>
          <div className="glass-surface-vibrant p-6 rounded-[32px] border border-white/10">
            <p className="text-[9px] uppercase tracking-widest font-black opacity-30 mb-2">Daily Streak</p>
            <p className="text-2xl font-display font-black text-orange-500">1</p>
          </div>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-xl font-display uppercase tracking-tighter mb-4">BetterYou Sudoku</h3>
            <p className="text-xs opacity-50 leading-relaxed mb-8">
              A classic logic-based number placement puzzle. Perfect for sharpening your pattern recognition and logical thinking.
            </p>
            <button 
            ...
            >
              {hasPlayedToday ? (
                <div className="flex flex-col items-center justify-center gap-1">
                  <div className="flex items-center gap-2"><Lock size={14} /> {t.playedToday}</div>
                  <div className="text-[8px] opacity-60 font-mono tracking-tighter">{t.nextSessionIn}: {timeLeft}</div>
                </div>
              ) : t.startTraining}
            </button>
          </div>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-xl font-display uppercase tracking-tighter mb-4">Focus Flow</h3>
            <p className="text-xs opacity-50 leading-relaxed mb-8">
              A spatial reasoning puzzle designed to reset your mental state and prepare you for the next deep work session.
            </p>
            <button 
          ...
          >
            <div className="max-w-2xl w-full">
              <div className="text-center mb-12">
                <div className="flex items-center justify-center gap-4 mb-4">
                  ...
                  <h2 className="text-5xl font-display uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#ff4e00] to-purple-500">
                    {currentGame === 'sudoku' ? 'BetterYou Sudoku' : 'Focus Flow'}
                  </h2>
                  <Sparkles className="text-[#ff4e00] animate-pulse" />
                </div>
                <p className="text-[10px] uppercase tracking-[0.3em] font-bold opacity-40">Cognitive Calibration in Progress</p>
              </div>
              ...
              <div className="glass-surface-vibrant rounded-[50px] p-12 border border-white/10 shadow-2xl relative overflow-hidden">
                ...
                {currentGame === 'sudoku' ? (
                  <SudokuGame onComplete={logPlay} onCancel={() => setCurrentGame(null)} stats={stats} updateStats={updateStats} />
                ) : (
                  <FocusFlowGame onComplete={logPlay} onCancel={() => setCurrentGame(null)} stats={stats} updateStats={updateStats} />
                )}
              </div>
              ...
              <div className="mt-12 flex items-center justify-center gap-6 opacity-40">
                <div className="flex items-center gap-2">
                  <AlertCircle size={14} />
                  <span className="text-[10px] uppercase tracking-widest">Daily session will be logged upon completion</span>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <div className="p-10 bg-gradient-to-br from-[#ff4e00]/10 via-transparent to-purple-500/10 border border-white/10 rounded-[40px] flex items-start gap-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h4 className="text-sm font-bold uppercase tracking-widest text-[#ff4e00] mb-3">The Golden Rule</h4>
          <p className="text-xs leading-relaxed opacity-60 italic max-w-2xl">
            "Mini-games are designed as a 'Cognitive Reset'. To prevent addiction and ensure they remain a productive break, you are limited to one session per day. Use it when you truly feel the mental fatigue setting in."
          </p>
        </div>
      </div>
    </div>
