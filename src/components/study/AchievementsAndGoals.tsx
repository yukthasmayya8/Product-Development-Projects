...
const BADGES = [
  ...
  },
  {
    ...
    description: "Generated flashcards for 5 different subjects.",
  },
];

export const AchievementsAndGoals: React.FC<{ language: string }> = ({
  language,
}) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeGoals = onSnapshot(goalsQuery, (snapshot) => {
      setGoals(
        snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as StudyGoal,
        ),
      );
    });
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          {BADGES.map((badge) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-4 rounded-full bg-white/5 mb-4 ${isEarned ? badge.color : "text-gray-500"}`}
                >
                  <Icon size={32} />
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {["daily", "weekly"].map((p) => (
                      <button
                        key={p}
                        onClick={() =>
                          setNewGoal({ ...newGoal, period: p as any })
                        }
