          ...
          <div className="flex justify-between items-center mb-12">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <h3 className="text-[10px] uppercase tracking-[0.4em] font-sans font-black text-white/40">
                  {t.timetable}
                </h3>
                <span className="text-xs uppercase tracking-widest font-black text-[#ff4e00] drop-shadow-lg">
                  {t[duration] || duration}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-4">
              ...
              {!isEditing && subjects.length > 0 && (
                <div className="relative group/select">
                  ...
                  >
                    <option value="" className="bg-[#0a0502]">
                      Focus Subject...
                    </option>
                    {subjects.map((s) => (
                      <option key={s} value={s} className="bg-[#0a0502]">
                        {s}
                      </option>
                    ))}
                  </select>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={() =>
                  isEditing ? saveTimetable() : setIsEditing(true)
                }
                disabled={loading}
              ...
              >
                {isEditing ? (
                  <Save size={20} className="text-[#ff4e00]" />
                ) : (
                  <Edit3 size={20} className="opacity-40" />
                )}
              </motion.button>
              {!isEditing && (
                <motion.button
                ...
                >
                  <RefreshCw
                    size={20}
                    className={
                      loading ? "animate-spin text-[#ff4e00]" : "opacity-40"
                    }
                  />
                </motion.button>
              )}
            ...
            {loading && !isEditing ? (
              ...
              >
                <div className="relative w-20 h-20 mb-10">
                  ...
                  <Sparkles
                    size={24}
                    className="absolute inset-0 m-auto text-[#ff4e00] animate-pulse"
                  />
                </div>
                <p className="text-[10px] uppercase tracking-[0.5em] font-black text-transparent bg-clip-text bg-gradient-to-r from-white/40 to-white/10 animate-pulse">
                  Architecting Schedule
                </p>
              </motion.div>
            ) : schedule.length > 0 ? (
              <Reorder.Group
                ...
                {schedule.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="w-28 flex flex-col items-center justify-center py-5 bg-black/60 rounded-3xl border border-white/10 shadow-2xl relative z-10">
                      <Clock
                        size={14}
                        className="text-[#ff4e00] mb-2 opacity-60"
                      />
                      {isEditing ? (
                        <input
                          type="text"
                          value={item.time}
                          onChange={(e) =>
                            handleEditChange(idx, "time", e.target.value)
                          }
                        ...
                        <span className="text-xs font-black tracking-tight tabular-nums text-white/90">
                          {item.time}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex-1 relative z-10">
                      <div className="flex items-center gap-4 mb-3">
                        {isEditing ? (
                          <input
                            ...
                            value={item.activity}
                            onChange={(e) =>
                              handleEditChange(idx, "activity", e.target.value)
                            }
                          ...
                          <h4 className="text-xl font-black tracking-tight text-white group-hover:text-[#ff4e00] transition-colors">
                            {item.activity}
                          </h4>
                        )}
                      ...
                      {isEditing ? (
                        <textarea
                          value={item.tip}
                          onChange={(e) =>
                            handleEditChange(idx, "tip", e.target.value)
                          }
                ...
                {isEditing && (
                  ...
                  >
                    <div className="p-4 bg-white/5 rounded-2xl group-hover:bg-[#ff4e00]/20 transition-all">
                      <Plus
                        size={24}
                        className="text-white/20 group-hover:text-[#ff4e00]"
                      />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 group-hover:text-white/60">
                      {t.addBlock}
                    </span>
                  </motion.button>
                )}
              ...
              <div className="text-center py-32 opacity-20 italic">
                <p className="text-sm tracking-[0.5em] uppercase font-black">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-3xl font-display uppercase tracking-tighter text-[#ff4e00]">
                    {t.uploadSyllabus}
                  </h3>
                  <p className="text-[10px] uppercase tracking-widest font-bold opacity-40">
                    Tailor your focus path
                  </p>
                </div>
                <button
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                      <h4 className="font-bold text-sm mb-2 group-hover:text-[#ff4e00] transition-colors">
                        Exam Focus
                      </h4>
                    ...
                    >
                      <h4 className="font-bold text-sm mb-2 group-hover:text-purple-400 transition-colors">
                        Mastery Mode
                      </h4>
                      <p className="text-[10px] opacity-50 leading-relaxed">
                        {t.masteryMode}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {reminders.map((reminder) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-2 rounded-xl ${reminder.active ? "bg-blue-500/20 text-blue-500" : "bg-white/5 text-gray-500"}`}
                >
                ...
                <div>
                  ...
                  {reminder.topic && (
                    <p className="text-[10px] text-[#ff4e00] font-bold uppercase tracking-widest mb-1">
                      {reminder.topic}
                    </p>
                  )}
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <RefreshCw
                    size={14}
                    className={reminder.active ? "text-blue-400" : "opacity-40"}
                  />
                </button>
                <button
