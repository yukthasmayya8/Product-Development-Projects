...
export default function Layout({ children, user, language = 'English' }: LayoutProps) {
          ...
          <div className="flex justify-between h-24 items-center">
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] uppercase tracking-[0.5em] font-black text-white/20 group-hover:text-[#ff4e00] transition-colors duration-500">Master Your Mind</span>
              </div>
            </motion.div>
            ...
            {user && (
              <div className="flex items-center gap-10">
                <div className="hidden lg:flex items-center gap-10 text-[10px] font-black uppercase tracking-[0.3em] text-white/30">
                  <motion.span whileHover={{ scale: 1.05, color: '#fff' }} className="flex items-center gap-2.5 transition-all cursor-pointer"><Clock size={14} className="text-[#ff4e00]" /> {t.focus}</motion.span>
                  <motion.span whileHover={{ scale: 1.05, color: '#fff' }} className="flex items-center gap-2.5 transition-all cursor-pointer"><Calendar size={14} className="text-[#ff4e00]" /> {t.schedule}</motion.span>
                  <motion.span whileHover={{ scale: 1.05, color: '#fff' }} className="flex items-center gap-2.5 transition-all cursor-pointer"><Heart size={14} className="text-[#ff4e00]" /> {t.mood}</motion.span>
                </div>
                <div className="flex items-center gap-6 pl-10 border-l border-white/10">
                  <div className="flex flex-col items-end">
                    <span className="text-[9px] font-black uppercase tracking-[0.4em] text-[#ff4e00] drop-shadow-lg">{t.pioneer}</span>
                    <span className="text-sm font-black tracking-tight text-white/90">{user.displayName}</span>
                  </div>
                  <motion.button 
      ...
      <footer className="border-t border-white/5 py-16 mt-auto bg-[#0a0502]/80 backdrop-blur-3xl relative z-10">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="flex items-center gap-4 group cursor-pointer">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white/20 group-hover:text-white/40 transition-colors">{t.forTheBetterYou}</span>
          </div>
          ...
          <div className="flex gap-10 text-[9px] font-black uppercase tracking-[0.4em] text-white/20">
            <a href="#" className="hover:text-[#ff4e00] transition-colors">{t.privacy}</a>
            <a href="#" className="hover:text-[#ff4e00] transition-colors">{t.terms}</a>
          </div>
        </div>
      </footer>
    </div>
