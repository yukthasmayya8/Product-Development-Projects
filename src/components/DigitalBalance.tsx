...
import { Smartphone, BookOpen, Award, TrendingDown, Info, CheckCircle2, Zap, RefreshCw, Save } from 'lucide-react';
import { db, doc, setDoc, Timestamp, handleFirestoreError, OperationType } from '../firebase';
...
export default function DigitalBalance({ userId, studyMinutes, loggedScreenTime = 0, language = 'English', onUpdate, loading = false }: DigitalBalanceProps) {
  const [screenTime, setScreenTime] = useState<string>(loggedScreenTime > 0 ? loggedScreenTime.toString() : '');
  ...
  const productivityRatio = studyMinutes > 0 ? (studyMinutes / (totalScreenTime || 1)).toFixed(2) : "0.00";
  const isBetterYouMaster = studyMinutes > totalScreenTime && totalScreenTime > 0;
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-display uppercase tracking-tighter">{t.digitalBalance}</h3>
              <p className="text-[10px] uppercase tracking-widest font-bold opacity-40">{t.transcendNoise}</p>
            </div>
          </div>
          {isBetterYouMaster && (
            <motion.div 
          ...
          <div className="space-y-6">
            <div className="bg-white/5 p-8 rounded-[32px] border border-white/10 relative overflow-hidden group/card hover:bg-white/10 transition-all">
              ...
              <div className="flex items-center justify-between mb-4 relative z-10">
                <span className="text-[10px] uppercase tracking-widest font-bold opacity-30">{t.studyTime}</span>
              ...
              <div className="flex items-baseline gap-2 relative z-10">
                {loading ? (
                  ...
                  <>
                    <span className="text-4xl font-display uppercase tracking-tighter text-blue-400">{studyMinutes}</span>
                    <span className="text-[10px] font-bold opacity-30 uppercase">{t.minutes}</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="bg-white/5 p-8 rounded-[32px] border border-white/10 relative overflow-hidden group/card hover:bg-white/10 transition-all">
              ...
              <div className="flex items-center justify-between mb-4 relative z-10">
                <span className="text-[10px] uppercase tracking-widest font-bold opacity-30">External Screen Time</span>
                <motion.button 
              ...
              <p className="text-[8px] opacity-20 mt-3 italic relative z-10">Sync manually from your phone's Digital Wellbeing</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#ff4e00]/20 to-purple-500/10 p-8 rounded-[32px] border border-[#ff4e00]/30 glow-orange relative overflow-hidden group/card">
              ...
              <div className="flex items-center justify-between mb-4 relative z-10">
                <span className="text-[10px] uppercase tracking-widest font-bold text-[#ff4e00]">Active App Usage</span>
              ...
              <div className="flex items-baseline gap-2 relative z-10">
                {loading ? (
                  ...
                  <>
                    <span className="text-4xl font-display uppercase tracking-tighter text-[#ff4e00]">{activeAppMinutes}</span>
                    <span className="text-[10px] font-bold opacity-30 uppercase text-[#ff4e00]">{t.minutes}</span>
                  </>
                )}
              </div>
              <p className="text-[8px] opacity-40 mt-3 italic relative z-10">Real-time tracking of your focus on BetterYou</p>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-center items-center text-center p-10 bg-white/5 rounded-[50px] border border-white/10 relative overflow-hidden group/card">
                ...
                <>
                  <svg className="w-56 h-56 transform -rotate-90 relative z-10">
                    ...
                    <motion.circle
                      ...
                      animate={{ strokeDashoffset: 628.3 - (Math.min(parseFloat(productivityRatio), 2) / 2) * 628.3 }}
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                    ...
                    <span className="text-[10px] font-black opacity-30 uppercase tracking-[0.3em] mt-3">{t.productivityRatio}</span>
                  </div>
                </>
              )}
            </div>
            {loading ? (
              ...
              <p className="text-xs opacity-60 italic leading-relaxed max-w-[240px] relative z-10">
                {isBetterYouMaster ? t.appreciation : "Strive to keep your study time higher than your screen time for maximum growth."}
              </p>
            )}
          </div>
        </div>
        ...
        <AnimatePresence>
          {isBetterYouMaster && (
            ...
            >
              ...
              <div className="relative z-10">
                <h4 className="text-lg font-display uppercase tracking-tighter text-green-400 mb-2">{t.rewardUnlocked}</h4>
                <p className="text-sm opacity-60 italic">{t.betterYouMasterDesc}</p>
              </div>
        ...
        <div className="mt-12 pt-12 border-t border-white/5 grid grid-cols-1 sm:grid-cols-3 gap-10">
          <div className="flex items-start gap-4 group/tip">
            ...
            <div>
              <h5 className="text-[11px] font-bold uppercase tracking-widest mb-2">Reduce Noise</h5>
              <p className="text-[10px] opacity-40 leading-relaxed">Turn off non-essential notifications during focus blocks.</p>
            </div>
          </div>
          <div className="flex items-start gap-4 group/tip">
            <div className="mt-1 p-3 bg-white/5 rounded-2xl border border-white/10 group-hover/tip:bg-[#ff4e00]/10 group-hover/tip:border-[#ff4e00]/20 transition-all">
            ...
            <div>
              <h5 className="text-[11px] font-bold uppercase tracking-widest mb-2">Grey Scale</h5>
              <p className="text-[10px] opacity-40 leading-relaxed">Set your phone to grayscale to make it less addictive.</p>
            </div>
          </div>
          <div className="flex items-start gap-4 group/tip">
            <div className="mt-1 p-3 bg-white/5 rounded-2xl border border-white/10 group-hover/tip:bg-[#ff4e00]/10 group-hover/tip:border-[#ff4e00]/20 transition-all">
            ...
            <div>
              <h5 className="text-[11px] font-bold uppercase tracking-widest mb-2">Physical Distance</h5>
              <p className="text-[10px] opacity-40 leading-relaxed">Keep your phone in another room while studying.</p>
            </div>
          </div>
        </div>
      </div>
