...
export default function FocusAnalytics({ studyData, screenTimeData, distractionData, language = 'English', loading = false }: FocusAnalyticsProps) {
  ...
  const chartData = last7Days.map(date => {
    const dayStudy = studyData
      .filter(s => {
        const sDate = s.timestamp?.toDate ? s.timestamp.toDate().toISOString().split('T')[0] : s.date;
        return sDate === date;
      })
      .reduce((acc, curr) => acc + (curr.duration || curr.durationMinutes || 0), 0);

    const dayScreen = screenTimeData.find(s => s.date === date)?.screenTime || 0;
  ...
  const totalDistractionTime = distractionData.reduce((acc, curr) => acc + (curr.duration || 0), 0);
  const averageRatio = totalStudy > 0 ? (totalStudy / (totalScreen || 1)).toFixed(2) : "0.00";
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[8px] uppercase tracking-widest font-bold opacity-40">{t.studyTime}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-white/20" />
              <span className="text-[8px] uppercase tracking-widest font-bold opacity-40">{t.screenTime}</span>
            </div>
          </div>
        </div>
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  />
                  <Bar dataKey="study" fill="#ff4e00" radius={[4, 4, 0, 0]} barSize={12} />
                  <Bar dataKey="screen" fill="rgba(255,255,255,0.1)" radius={[4, 4, 0, 0]} barSize={12} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div className="space-y-6 flex flex-col justify-center">
            <div className="p-6 bg-white/5 rounded-[32px] border border-white/10">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[9px] uppercase tracking-widest font-bold opacity-30">Weekly Ratio</span>
              ...
              <div className="flex items-baseline gap-2">
                {loading ? (
                  ...
                  <>
                    <span className="text-3xl font-display tracking-tighter">{averageRatio}</span>
                    <span className="text-[8px] font-bold opacity-30 uppercase">Study/Screen</span>
                  </>
                )}
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-6 bg-white/5 rounded-[32px] border border-white/10">
                <span className="text-[9px] uppercase tracking-widest font-bold opacity-30 block mb-4">Top Distractions</span>
                <div className="space-y-3">
                  {topDistractions.map(([source, count]: any) => (
                    <div key={source} className="flex items-center justify-between">
                      <span className="text-[10px] font-bold opacity-60">{source}</span>
                      <span className="text-[10px] font-mono text-[#ff4e00]">{count}x</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="p-6 bg-[#ff4e00]/5 rounded-[32px] border border-[#ff4e00]/10">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] uppercase tracking-widest font-bold text-[#ff4e00]">Insight</span>
              </div>
              {loading ? (
          ...
          {loading ? (
            ...
            <>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
                ...
                <div className="text-lg font-display tracking-tighter">{totalStudy}</div>
                <div className="text-[8px] uppercase tracking-widest font-bold opacity-30">Total Study</div>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
                <Smartphone size={14} className="mx-auto mb-2 opacity-20" />
                <div className="text-lg font-display tracking-tighter">{totalScreen}</div>
                <div className="text-[8px] uppercase tracking-widest font-bold opacity-30">Total Screen</div>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
                <Activity size={14} className="mx-auto mb-2 opacity-20" />
                <div className="text-lg font-display tracking-tighter">{totalDistractionTime}</div>
                <div className="text-[8px] uppercase tracking-widest font-bold opacity-30">Total Distraction</div>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
                <Activity size={14} className="mx-auto mb-2 opacity-20" />
                <div className="text-lg font-display tracking-tighter">{Math.round(totalStudy / 7)}</div>
                <div className="text-[8px] uppercase tracking-widest font-bold opacity-30">Avg Study/Day</div>
              </div>
            </>
          )}
        </div>
